...
本版本把“药品库存身份”和“实体分类柜”明确分层：SQLite 与 CloudBase 仍以
`hardware_slot=1..23` 标识 23 项固定药品，旧记录、计划、库存和同步键均不迁移；
本地 `cabinet_v2_catalog.py` 再按稳定药品 ID 把这些逻辑项投影到
`cabinet_id=1..3`。未配置映射的药品会停止取药，不会猜测柜位。

药品页和管理员维护页只展示三个实体分类柜。v2.0.0 的实机摆药清单已确认为：

| 分类柜 | 固定药品 |
| --- | --- |
| 1号「口服药品」（14 项） | 复方感冒灵颗粒、多维元素片、蒙脱石散、阿莫西林胶囊、蜜炼川贝枇杷膏、乳果糖口服液、银黄颗粒、藿香正气丸、双歧杆菌三联活菌肠溶胶囊、铝碳酸镁咀嚼片、布洛芬缓释胶囊、磷酸奥司他韦胶囊、苯磺酸氨氯地平片、枸地氯雷他定胶囊 |
| 2号「外用药品」（6 项） | 桂林西瓜霜、莫匹罗星软膏、酮康唑乳膏、碘伏消毒液、布地奈德鼻喷雾剂、酮洛芬凝胶 |
| 3号「医疗护理用品」（3 项） | 医用纱布敷料、创口贴、医用棉签 |

该变化没有修改 `cloudbase/`，三个 `cabinet_id` 也不会上传替代原有
`hardware_slot`。

实体执行链路为 QSM USB Host → ST-LINK VCP `/dev/ttyACM0` → STM32 USART2，
使用 115200 8N1、CRLF 文本协议。后端只发送 `CABINET 1|2|3` 点亮指示灯；
用户自行打开亮灯柜门取药，明确确认后系统发送 `OFF` 并用 `STATUS OFF`
复核。旧 `/dev/ttyS5`、9600、单字节 `slot/control_code` 开柜链路已停用。
...
- 今日计划、AI 问询和药品页自助取药是三条隔离路径。药品页先确认已登记人物，再由后端核查人物档案、药品审核事实、库存、逻辑身份、分类柜映射和有效期；只有短时 `PASSED` 结果在用户再次确认说明后才能点亮对应分类柜。`BLOCKED`、`CHECK_FAILED`、访客或资料变化都不会调用 QSM。
...
固定 23 项逻辑库存身份的库存值表示“是否还有药”，不是逐盒递减计数：正常状态稳定为 `stock=1 / AVAILABLE`，真实亮灯与人工取药不会自动减为零。现场用户可在确认页明确选择“还有药/已经用完”；前者保持 1，只有后者把库存写为 `stock=0 / DEPLETED`。确认操作与本次真实取药记录一起写入幂等账本，倒计时结束不会替用户判断已经用完。`hardware_slot=1..23` 继续同步，三个本地 `cabinet_id` 不替换云端身份。首页体征保持未归属；已登记人物的问询内体征由后端按持久化会话绑定人物和 `persona_generation`，访客问询内体征则只绑定该次问询与访客名快照，绝不伪装成登记人物。问询与体征上传的是有界历史窗口，因此只追加/更新，绝不对这两类调用 `FINALIZE_SNAPSHOT` 删除窗口外旧记录。
