...
问询模型可根据当前对话决定是否需要测量体征，并说明测量原因。额温、心率和血氧只有在设备质量合格时才作为核心体征；指温、估算血压、呼吸、HRV 等只作为辅助参考，不单独触发硬性风险或药品选择。测量完成、部分完成、失败或取消都会直接写回同一问询会话，由专用云端/离线融合步骤继续问询或分析；自动复测最多两次。
