...
CloudBase 2.6 要求所有健康数据读取先通过 ACTIVE membership，并按人物范围隔离人物、计划、问询、记录、体征和安全事件；安全事件还要求 `READ_SAFETY`。问询详情沿用同一范围，越权不泄露是否存在。通用命令的 request ID 在事务中绑定请求摘要，只读家属不能下发命令。小程序侧只有只读安全事件能力；`OPEN_CABINET` 已从云端允许列表、内嵌 helper 和板端执行器中移除，历史命令也会失败关闭且不触碰 QSM。Station 上报必须携带服务端配置的设备密钥，缺少密钥时失败关闭。
