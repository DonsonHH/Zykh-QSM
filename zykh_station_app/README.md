...
固定 23 仓使用 `database-safety-facts-v6-repaired-fixed-catalog` 受控资料版本，全部进入问询安全评估。候选召回会把已记录的用户原话与开放式症状事实一起使用，并通过受控口语等价词和目录用途映射消除“清水鼻涕没有找到鼻炎药”等漏召回；不同的合适病例可覆盖不同药品，有两个直接相关且安全的选择时优先展示用途侧重不同的主方案和备选。当前目录回归证明 23 项在包装未过期且处方项具备有效既往计划的对应病例中均可进入候选并通过本地单药方案校验。实际取药仍会逐项检查实时库存、有效期、包装身份、说明资料、过敏禁忌以及处方/既往计划资格；过期、无既往计划的处方药或不满足其他硬性资格的药品不能为了覆盖目录而强行开柜。动态扫码或云端换药仍从 `draft` 开始，不能继承固定仓身份的审核状态。
