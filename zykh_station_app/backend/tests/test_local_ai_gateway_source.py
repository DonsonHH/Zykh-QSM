...
class LocalAiGatewaySourceTest(unittest.TestCase):
    ...
    def test_normal_gateway_scripts_do_not_probe_or_forward_the_legacy_model(self) -> None:
        ...
        self.assertIn('QSM_SAFE_STATUS_PATH="${QSM_SAFE_STATUS_PATH:-/api/audio/status}"', ensure_source)
        self.assertIn('curl -fsS --max-time 3 "${QSM_BASE_URL}${QSM_SAFE_STATUS_PATH}"', ensure_source)
        self.assertNotIn('"$QSM_BASE_URL/api/status"', ensure_source)
        ...
        self.assertIn('QSM_SAFE_STATUS_PATH="${QSM_SAFE_STATUS_PATH:-/api/audio/status}"', check_source)
        self.assertIn('"${QSM_BASE_URL}${QSM_SAFE_STATUS_PATH}"', check_source)
        self.assertNotIn('"${QSM_BASE_URL}/api/status"', check_source)
