...
MODEL_ARCHIVE="sherpa-onnx-streaming-zipformer-small-ctc-zh-int8-2025-04-01.tar.bz2"
...
tar -xjf "$CACHE_DIR/$MODEL_ARCHIVE" -C "$CACHE_DIR/model" --strip-components=1
MODEL_DIR="$CACHE_DIR/model/sherpa-onnx-streaming-zipformer-small-ctc-zh-int8-2025-04-01"
...
adb push \
  ...
  "$REMOTE_ROOT/lib/"
adb push "$MODEL_DIR/model.int8.onnx" "$MODEL_DIR/tokens.txt" "$REMOTE_ROOT/model/"
...
adb shell "QSM_LOCAL_ASR_ROOT='$REMOTE_ROOT' sh /userdata/zykh_app/scripts/start_local_asr.sh"
