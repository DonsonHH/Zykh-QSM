...
In v2.0.1, `NETWORK_DEMO_SIMULATE=true` is the temporary default. The host reads only its default route and WiFi link so the top bar can show the real WiFi state alongside the simulated three-bar 4G presentation. Startup and device checks use the side-effect-free `QSM_SAFE_STATUS_PATH`; the generic board `/api/status` no longer performs EC200A, route or ping probes, and the simulated branch does not call QSM network, ADB, AT or tether operations. WiFi cannot be disabled against that simulated backup. Set the flag to false before using the dedicated network endpoints below.

In both simulated-4G and real-network modes, WiFi strength comes from the host `iw ... link` dBm value. In real-network mode only, SIM strength comes from the QSM EC200A `AT+CSQ` response and is converted to dBm, percentage and 0-4 bars. The top bar therefore reflects each available link independently instead of treating temporary 4G presentation as evidence about WiFi.
