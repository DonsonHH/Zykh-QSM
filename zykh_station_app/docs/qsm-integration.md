...
The 8085 gateway owns one measurement session at a time. It runs UART8 and GY-614 reads concurrently and retries the UART start sequence once only when no valid protocol frame appears after two seconds. A session with no fingertip signal ends after 10 seconds. The module vendor specifies 5–10 seconds for the initial algorithm result and about 1.28 seconds for later updates, so a real contact/heart-rate signal opens a 12-second adaptive stabilization window. Two non-zero heart-rate/SpO2 frames finish early; recent values are aggregated independently. A much later SpO2 value is treated as unstable optical contact rather than extending every normal session. Heart rate, SpO2 and forehead temperature are all required for `complete`; blood pressure and HRV are never synthesized and do not delay completion. Each new session sends `0x2A`, clears stale UART input, then sends `0x24`. Cancellation, completion and read failure all stop the module with `0x2A`.
...
- `audio_asr()`
- `audio_status()`
- `audio_speak()` is retained only as a compatibility guard; offline TTS is generated by the host and sent through the PCM playback stream.
- `audio_beep()`
