...
const EMPTY_PLAN = { time: "08:00", service_user_id: "", medicine_id: "", dose: "1片", status: "待执行" };
  ...
  return (
    <div className="admin-view admin-plans-view">
      <div className="admin-page-heading">
        <div><h2>今日用药</h2><p>计划只关联当前服务对象与药柜内真实药品</p></div>
      ...
      <div className="admin-split-view plans-split-view">
        <section className="admin-list-panel admin-plan-list-panel">
          <div className="admin-plan-list-header"><span>时间</span><span>服务对象</span><span>药品</span><span>状态</span></div>
          <div className="admin-plan-list-scroll">
            {data.plans.map((plan) => (
              <button key={plan.id} type="button" className={plan.id === selectedId && !creating ? "active" : ""} onClick={() => { setCreating(false); setSelectedId(plan.id); }}>
                <time>{plan.time}</time><strong>{plan.target_user}</strong><span>{plan.medicine}</span><small>{plan.status}</small>
              </button>
            ))}
          ...
          {(creating || selected) ? (
            <>
              <div className="admin-form-grid plan-editor-grid">
                ...
                <label><span>状态</span><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option>待执行</option><option>已执行</option><option>已跳过</option></select></label>
              ...
              <footer className="admin-editor-actions">
                ...
                <button type="button" className="admin-button primary" onClick={save} disabled={busy || !form.service_user_id || !form.medicine_id}><Save size={17} />{busy ? "保存中" : "保存计划"}</button>
              </footer>
            </>
