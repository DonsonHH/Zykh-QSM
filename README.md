...
- 双歧杆菌三联活菌肠溶胶囊（本地 S09）实体放在 3 号柜，Station 板端投影为 `storageBox=PRESCRIPTION`；板端快照为 `DAILY/CARE/PRESCRIPTION = 9/8/6`，不再生成单独的 `COLD` 分类。
- 本轮没有修改 `zykh_station_app/cloudbase/` 或外部 Zykh-Miniprogram 代码。板端传输适配负责本地 S03 蒙脱石/S13 布洛芬与小程序 canonical S13 蒙脱石/S03 布洛芬之间的双向转换；CloudBase 不能远程开柜或点灯。当前外部小程序 `origin/main` 仍把 S09 固定为 `COLD`，因此本版本只证明 Station 出站投影，不能宣称小程序界面已经同步；小程序须另行修改、验证和发布。
