...
DEMO_MEDICINES = [
    ...
    (2, "蒙脱石散", "3g/袋", 18, "2027-03-31", "869000100002", "big", "肠胃", "腹泻,肠胃不适", "高热、便血、严重脱水需立即就医", "按说明书核对", "腹泻超过24小时或老人儿童需管理员复核", "袋", 1, 0),
    ...
    (5, "氯雷他定片", "10mg/片", 20, "2027-01-31", "869000100005", "big", "过敏", "皮肤瘙痒,过敏性鼻炎", "孕妇、肝肾功能异常或儿童需医生指导", "按说明书核对", "呼吸困难、面唇肿胀属于紧急风险", "片", 1, 0),
    ...
    (11, "创可贴", "20片/盒", 25, "2027-06-30", "869000100011", "small", "外伤消毒", "轻微擦伤,伤口保护", "大面积伤口、动物咬伤需医生处理", "清洁消毒后使用", "只用于轻微外伤临时处理", "盒", 1, 0),
]
...
DEMO_RECORDS = [
    ("dispense", 7, "张三 · 硝苯地平控释片", "success", "固定服务对象 18:30 计划已取药确认"),
    ("vitals_read", 0, "李四 · 体征测量", "success", "心率72，血氧98%，体温36.5℃"),
    ("medicine_scan", 4, "连花清瘟胶囊", "success", "现场对象药盒拍照核验，置信度98%"),
    ("ai_triage", 0, "王五 · 腹泻一天", "review", "AI 应急问询已生成低风险记录，待值守员抽查"),
]
...
def seed_demo_data() -> None:
    ...
    with connect() as conn:
        ...
        conn.execute(
            ...
            ("weak", "rules", "村镇弱网模式演示，云端超时后切换本地模型或规则兜底", now),
        )
