...
if [ -n "$USB_CAMERA" ] && [ "$ZYKH_CAMERA_DEVICE" = "$USB_CAMERA" ]; then
  ...
  export ZYKH_CAMERA_PREVIEW_FPS="${ZYKH_CAMERA_PREVIEW_FPS:-20}"
  ...
  export ZYKH_CAMERA_PREVIEW_FPS="${ZYKH_CAMERA_PREVIEW_FPS:-15}"
...
nohup env \
  ...
  ZYKH_CAMERA_PREVIEW_FPS=$ZYKH_CAMERA_PREVIEW_FPS \
...
if [ -n "$PID" ]; then
    ...
    echo "camera: $ZYKH_CAMERA_DEVICE ${ZYKH_CAMERA_WIDTH}x${ZYKH_CAMERA_HEIGHT}@${ZYKH_CAMERA_FPS} preview=${ZYKH_CAMERA_PREVIEW_FPS}"
else
